...
then 
    ...
    do
        ...
        ERROR=`    cat .joblist.tmp | awk '{if (($3 == "Aborted") || ($5 != 0)) print}' | grep -v "Submitted\|Scheduled\|Running" | wc -l`
...
then
    ...
    do
        ...
        nAborted=`cat $TASK/status.tmp | grep -E "^[0-9]" | awk '{if (($3 == "Aborted") || ($5 != 0)) print}' | grep -v "Submitted\|Scheduled\|Running" | grep "Aborted" | wc -l`
        if [[ $nAborted != 0 ]]
        then 
...
then
    for TASK in $TASKS
    do
        cat $TASK/status.tmp | grep -E "^[0-9]" | awk '{if (($3 == "Aborted") || ($5 != 0)) print}' | grep -v "Submitted\|Scheduled\|Running" > resubmitList.tmp
